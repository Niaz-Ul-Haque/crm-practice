...
export default function ChatWidget() {
  ...
  const handleLlamaApiResponse = async (userMessage: string) => {
    try {
      ...
      const extractedEntities = extractEntitiesFromQuery(userMessage);

      const querySpecificData = prepareQuerySpecificData(
        userMessage,
        extractedEntities
      );

      const crmContext = `
        You are an AI assistant for an insurance CRM with ${clientsData.length} clients, 
        ${policiesData.length} policies, ${tasksData.length} tasks, and 
        ${opportunitiesData.length} opportunities.
        ...
        - If the data doesn't contain certain information, acknowledge its absence
        - Format currency values properly (e.g., $1,234.56)
        - Keep explanations brief and focused on facts from the data
      `;
      ...
      const apiMessages = [
        {
          role: 'system',
          content:
            'You are LynIQ, an AI assistant specialized in insurance CRM systems for financial advisors. ONLY respond to questions related to insurance, financial advising, client management, or the CRM system itself. If asked about topics unrelated to insurance, financial advising, or CRM usage, politely inform the user that you can only assist with insurance and financial advisory related matters. Do give your response in just text format and avoid usage of "*".',
        },
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        content:
          'Sorry, I encountered an error processing your request. Please try again with a more specific question.',
        sender: 'bot',
        timestamp: new Date(),
      };
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      >
        ...
        >
          ...
          {/* Show different content based on chat state */}
        ...
        {/* Green dot positioned relative to the button */}
        <motion.div
